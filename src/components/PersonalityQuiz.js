...
export const strengthSynonyms = {
	...
	"simulations": ["simulation", "simulations", "physics", "real-time", "interactive simulation", "virtual environment"],
...
const branchingQuestions = {
	...
	Presentira: [
		{
			text: "The Slide Seraph, who crafts dazzling slide decks for intergalactic audiences.",
			scores: {
				"strengths-slides": 1,
				"category-visualization & design": 3,
			}
	...
	Dimensio: [
		...
		{
			text: "The Generative Griffin, who conjures 3D models from text prompts.",
			scores: {
				"ai-enthusiast": 3,
				"quick-starter": 2,
				"rapid-prototyper": 2,
	...
	Researa: [
		...
		},
		{
			...
			scores: {
				...
				"strengths-collaboration": 5,
				"strengths-community": 3,
				"strengths-real-time-collaboration": 5
			}
		}
	]
};
...
const baseQuizQuestions = [
		...
		options: [
			{ 
				...
				scores: { 
					...
					"self-directed": 3, //no tools mapped to this...
					"focus-driven": 2
				} 
			},
			{
				...
				scores: {
					...
					"brainstormer": 2,
					"social-creator": 2,
					"strengths-real-time-collaboration": 2,
					"strengths-collaborative": 2
				}
			},
			{ 
		...
		options: [
			{
				...
				scores: {
					...
					"rapid-prototyper": 2,
					"experimental": 3,
					"idea-tester": 3, //not mapped to any tools...
					"early-adopter": 1
				}
			},
			{
		...
		options: [
			...
			},
			{
				...
				scores: {
					"quick-starter": 1,
					"rapid-prototyper": 1,
					"freeTierAccess-somewhat-limited": 1,
					"price-freemium": 2,
					"freeTierAccess-robust": 1,
					"price-free": 2,
					"freeTierAccess-very-limited": 1,
					"price-paid": 1
				}
			}
		],
	...
	},
	{
		...
		options: [
			{
				text: "Summon templates from the ether and customize at warp speed",
				scores: {
					"timePressure-low": 3,
			...
			},
			{
				...
				scores: {
					...
					"self-directed": 2, //no tools mapped to this...
					"artistic": 1,
					"timePressure-high": 3
				}
			},
...
const PersonalityQuizApp = () => {
   ...
   const handleAnswer = (optionIndex) => {
	...
	const calculateRecommendations = (testUserScores = null, testQuestions = null, testAnswers = null) => {
		...
		const q2Strengths = [
			'research', 'writing', 'content-creation', 'note-taking', 'brainstorming',
			'collaboration', 'organization', 'data-analysis', 'visualization',
			'coding', 'debugging', 'slides', '3d-modeling', 'game-design',
			'architecture', 'rendering', 'simulations', 'animation', 'audio-editing',
			'music-production', 'transcription', 'video-editing', 'documentation',
		...
		Object.keys(activeUserScores).forEach(key => {
			if (!key.startsWith('category-') && !key.startsWith('strengths-') &&
				...
				key !== 'collaborative-worker' && key !== 'solo-worker' && key !== 'hybrid-worker' &&
				key !== 'flexible-collaborator' && key !== 'quick-starter' && key !== 'efficiency-focused') {
		...
		// Collaboration preference (use max of collaborative/solo/hybrid)
		const maxCollabScore = Math.max(
			activeUserScores["collaborative-worker"] || 0,
			activeUserScores["solo-worker"] || 0,
			(activeUserScores["hybrid-worker"] || 0) > 0 ? 1 : 0,
			(activeUserScores["flexible-collaborator"] || 0) > 0 ? 1 : 0
		);
		if (maxCollabScore > 0) {
		...
		const toolScores = filteredTools.map(tool => {
			...
			// Collaboration vs Solo preference filtering (enhanced)
			const collaborationKeywords = ['collaborat', 'team', 'share', 'group', 'real-time collab', 'live edit', 'social'];
			const soloKeywords = ['individual', 'personal', 'private', 'offline', 'local'];
			...
			const isSoloTool = tool.strengths && tool.strengths.some(s =>
				soloKeywords.some(keyword => s.toLowerCase().includes(keyword))
			) || tool.shortDescription && soloKeywords.some(keyword => 
				tool.shortDescription.toLowerCase().includes(keyword)
			);
			
			// Strong collaboration preference
			if (activeUserScores['collaborative-worker'] > (activeUserScores['solo-worker'] || 0)) {
				if (isCollaborativeTool) {
					score += 3;
					reasons.collaborationPreference.push('Built for teamwork and real-time collaboration');
				} else if (isSoloTool) {
					score -= 3; // Penalty for solo tools when preferring collaboration
					totalPossiblePenalties += 3;
				}
			}

			// Strong solo preference
			if (activeUserScores["solo-worker"] > (activeUserScores["collaborative-worker"] || 0)) {
				if (isSoloTool) {
					score += 3;
					reasons.collaborationPreference.push('Ideal for independent, focused work');
				} else if (isCollaborativeTool) {
					score -= 2; // Light penalty for collaborative tools when preferring solo work
					totalPossiblePenalties += 2;
				}
			}

			// Balanced collaboration preference
			if (activeUserScores['hybrid-worker'] || activeUserScores['flexible-collaborator']) {
				if (isCollaborativeTool) {
					score += 1;
					reasons.collaborationPreference.push('Supports both solo and collaborative workflows');
				} else if (isSoloTool) {
					score += 1;
					reasons.collaborationPreference.push('Works well for flexible work styles');
				}
			}
			// Complexity mismatch penalties
		...
		const topScore = sortedForDebug.length > 0 ? Math.max(0, sortedForDebug[0].score) : 0;
		const topToolRawPercentage = universalMaxPossibleScore > 0 ? topScore / universalMaxPossibleScore : 0;
		const targetTopPercentage = 95;
		const scaleFactor = topToolRawPercentage > 0 ? targetTopPercentage / (topToolRawPercentage * 100) : 1;
		// Adjusted max is what the max would be if top score = 95%
		const adjustedMaxPossible = topScore > 0 ? (topScore / (targetTopPercentage / 100)) : universalMaxPossibleScore;

		console.log(`Adjusted Max Possible (for top tool at 95%): ${adjustedMaxPossible.toFixed(1)}`);
		...
		for (const tool of sortedTools) {
			...
			if (diverseTools.length >= 4) break; // Get top 4 for final selection
		}
		...
		// Calculate percentages normalized so top tool shows ~95%
		const finalRecommendations = sortedDiverseTools
			.slice(0, 2)
			.map((tool, index) => {
				let percentage;

				if (universalMaxPossibleScore > 0 && sortedDiverseTools.length > 0) {
					const effectiveScore = Math.max(0, tool.score);
					const topScore = Math.max(0, sortedDiverseTools[0].score);

					// Calculate what % of universal max the top tool achieved
					const topToolRawPercentage = topScore / universalMaxPossibleScore;

					// Scale factor: if top tool got 40% of universal max, scale so it shows as 95%
					// Target: top tool = 95%, other tools scale proportionally
					const targetTopPercentage = 95;
					const scaleFactor = topToolRawPercentage > 0 ? targetTopPercentage / (topToolRawPercentage * 100) : 1;

					// Calculate raw percentage for this tool
					const rawPercentage = (effectiveScore / universalMaxPossibleScore) * 100;

					// Apply scaling
					percentage = Math.round(rawPercentage * scaleFactor);

					// Ensure reasonable bounds (cap at 100%)
							...
							{!showResults ? (
								...
								<div className="result-card rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 mb-6 sm:mb-8" style={{
									...
									<div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
										{calculateRecommendations().map((tool, idx) => (
